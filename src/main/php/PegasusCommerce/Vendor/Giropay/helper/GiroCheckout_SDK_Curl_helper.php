...
class GiroCheckout_SDK_curl_helper {
    ...
    public static function submit($url, $params)
    {
        ...
        if(__GIROCHECKOUT_SDK_DEBUG__) GiroCheckout_SDK_Debug::getInstance()->logRequest(curl_getinfo($ch),$params);
        if(__GIROCHECKOUT_SDK_DEBUG__) GiroCheckout_SDK_Debug::getInstance()->logReply($result, curl_error($ch));
    ...
    private static function getHeaderAndBody($response) {
        ...
        $header = self::http_parse_headers(substr($response, 0, strpos($response,"\r\n\r\n")));
